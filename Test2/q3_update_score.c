...
int main(int argc, char **argv) {
     ...
     */
    fseek(fp, (game_num - 1)*sizeof(struct game) ,SEEK_SET);

    fread(&old_game, sizeof(struct game), 1, fp);

    if (strcmp(old_game.team, team) ！= 0){
        printf(MESSAGE);
        exit(1);
    }
    ...
    fwrite(&old_game, sizeof(struct game), 1, fp);
    fclose(fp);
